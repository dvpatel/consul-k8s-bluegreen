...
func (r *EndpointsController) registerServicesAndHealthCheck(
	...
	if hasBeenInjected(pod) {
		...
		if managedByEndpointsController {
			...
			r.Log.Info("Curt: create service registrations")
...
func (r *EndpointsController) createServiceRegistrations(
	...
	if tproxyEnabled {
		...
		checks, err := getServiceNameChecks(r.ConsulClient, proxyService.Name)
		if err != nil {
			return nil, nil, err
		}

		iptables, err := createRedirectTrafficConfig(proxyService, checks)
		if err != nil {
			return nil, nil, err
		}
